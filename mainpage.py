...
# Combine fields into one text input
text_combined = f"{company_name} {location} {company_profile} {description} {requirements} {employment_type} {benefits}"

# -------------------------------------------------
# Location Consistency Logic (NEW)
# -------------------------------------------------
# Hardcoded risky locations (top 10 from dataset analysis)
risky_locations = {
...
def check_location_consistency(loc):
    ...
    if loc in risky_locations:
        return 0   # mark as not consistent (risky location)
    return 1       # safe location
